    ...
    public class AutoReader : MonoBehaviour
    {
        ...
        private IEnumerator AutoRead()
        {
            ...
            while (conversationManager.isRunning)
            {
                if (!skip)
                {
                    while (!architect.isBuilding && !conversationManager.isWaitingOnAutoTimer) yield return null;
        ...
        public void Toggle_Auto()
        {
            ...
            if (isOn) statusText.text = STATUS_TEXT_AUTO;
        }
        public void Toggle_Skip()
        {
            ...
            if (isOn) statusText.text = STATUS_TEXT_SKIP;
        }
    }
}