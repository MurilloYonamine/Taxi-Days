...
namespace COMMANDS
{
    ...
    {
        private static string[] PARAM_SFX = new string[] { "-sfx" };
        ...
        private static void PlayVoice(string[] data)
        {
            ...
            if (sound == null)
            {
                Debug.Log("Audio clip não foi encontrado em: " + filepath);
                return;
            }
        ...
        {
            float volumeCap, pitch, startVolume;
            bool loop;
        ...
        {
            if(data == string.Empty) StopTrack("1");
            else StopTrack(data);
        }
        private static void StopAmbience(string data)
        {
            if (data == string.Empty) StopTrack("0");
            else StopTrack(data);
        }
